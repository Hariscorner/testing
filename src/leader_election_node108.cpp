...
void leaderCheck(const std_msgs::Int16MultiArray& msg){
	if(mymessage.data[0] == msg.data[1] && mymessage.data[1] == msg.data[1] && nround == diam){
		//when UID is equal to my max know UID which is equal to my neighbour's max known UID and the no:of rounds reached diameter. then self-proclaim myself as leader of the gang
		ROS_INFO_STREAM("My UID: " << mymessage.data[0] 
						<< ", Round no: " << nround 
